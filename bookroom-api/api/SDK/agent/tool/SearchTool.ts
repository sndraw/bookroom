import { SearchRequest, SearchResult, SearchAdapter, SearchEngineType, SEARCH_API_MAP } from '../../../common/search';
...
export default class SearchTool implements Tool {
  ...
  private initAdapters(): void {
    ...
    if (engineType && typeof engineType === 'string') {
      ...
      // 根据引擎类型初始化对应的适配器
      if (engineType === SEARCH_API_MAP.Tavily) {
        if (this.config.apiKey) {
          console.log(`[SearchTool] 初始化Tavily适配器成功`);
          this.engineAdapters.set(SEARCH_API_MAP.Tavily, new TavilyAdapter(this.config.apiKey));
        } else {
          console.log(`[SearchTool] 无法初始化Tavily适配器，缺少API密钥`);
        }
      } else if (engineType === SEARCH_API_MAP.weather) {
        if (this.config.apiKey) {
          console.log(`[SearchTool] 初始化Weather适配器成功`);
          this.engineAdapters.set(SEARCH_API_MAP.weather, new WeatherAdapter(this.config.apiKey));
        } else {
      ...
      console.log(`[SearchTool] 没有配置引擎类型`);
    }
    // 可以添加更多搜索引擎的初始化
